...
public class AddPhoto extends ActionBarActivity implements Camera.PreviewCallback{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		final MainActivity faceRecog =new MainActivity();
		       ...
		       public void onCheckedChanged(CompoundButton buttonView,boolean isChecked) {
		    	   ...
		    	   userName = Ed.getText().toString();
		    		hash = faceRecog.retrieveHash(getApplicationContext());
		    			...
		    			{
			    			  personId = Integer.parseInt(hash.get(userName));
			    			  if(personId>0)
			    			  {		
			    				  updatePerson = true;
			    			  }
		    			...
		    			Toast.makeText(getApplicationContext(), "Please enter a existing users name", Toast.LENGTH_SHORT).show();
		    			}
		       }
		   }
		);     
	...
	case 0:
		if (finalResultCode == RESULT_OK) {
			...
			if (bitmap != null) {
				...
				in1.putExtra(
						"dementiaHack.augmentedmemory.ImageConfirmation",
						byteArray);
				in1.putExtra(
						"dementiaHack.augmentedmemory.ImageConfirmation.switchCamera",
						true);
				in1.putExtra(
						"dementiaHack.augmentedmemory.ImageConfirmation.through.gallery",
						true);
				in1.putExtra(
						"dementiaHack.augmentedmemory.ImageConfirmation.orientation",
						0);
